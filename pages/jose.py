...
from app import app
df = pd.read_csv('pages/tendencies_analysis.csv')
...
def generate_graph(value):
    ...
    if '-' in value:
        ...
        fig.add_trace(go.Scatter(x = df['Year'], y = df[shottypea], name = 'Attempts'))
        fig.add_trace(go.Scatter(x = df['Year'], y = df[shottypem], name = 'Makes'))
